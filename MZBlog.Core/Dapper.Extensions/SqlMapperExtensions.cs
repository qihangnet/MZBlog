...
namespace Dapper.Extensions
{
    ...
    public static partial class SqlMapperExtensions
    {
        ...
        private static IEnumerable<PropertyInfo> ComputedPropertiesCache(Type type)
        {
            ...
            var computedProperties = TypePropertiesCache(type).Where(p => p.GetCustomAttributes(true).Any(a => a is ComputedAttribute));

            ComputedProperties[type.TypeHandle] = computedProperties;
            return computedProperties;
        }

        internal static IEnumerable<PropertyInfo> ExplicitKeyPropertiesCache(this Type type)
        {
            if (ExplicitKeyProperties.ContainsKey(type.TypeHandle))
            {
                return ExplicitKeyProperties[type.TypeHandle];
            }

            var explicitKeyProperties = TypePropertiesCache(type).Where(p => p.GetCustomAttributes(true).Any(a => a is ExplicitKeyAttribute));

            ExplicitKeyProperties[type.TypeHandle] = explicitKeyProperties;
            return explicitKeyProperties;
        }

        internal static IEnumerable<PropertyInfo> KeyPropertiesCache(this Type type)
        {
            ...
            lock (_locker)
            {
                ...
                {
                    var idProp = allProperties.FirstOrDefault(p => string.Equals(p.Name, "id", StringComparison.CurrentCultureIgnoreCase));
                    if (idProp?.GetCustomAttributes(true).Any(a => a is ExplicitKeyAttribute) == false)
                    {
        ...
        /// <param name="tableName">Special name of table</param>
        /// <returns>Entity of T</returns>
        public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null, string tableName = null) where T : class
        {
            var type = typeof(T);
        ...
        /// <param name="tableName">Special name of table</param>
        /// <returns>Entity of T</returns>
        public static IEnumerable<T> GetAll<T>(this IDbConnection connection, IDbTransaction transaction = null, int? commandTimeout = null, string tableName = null) where T : class
        {
            var type = typeof(T);
        ...
        internal static string GetTableName(this Type type)
        {
            ...
            else
            {
                ...
                if (type.GetTypeInfo()
                    .GetCustomAttributes(false).SingleOrDefault(attr => attr.GetType().Name == "TableAttribute") is TableAttribute tableAttr)
                {
                    name = tableAttr.Name;
                }
                else
        ...
        public static bool Update<T>(this IDbConnection connection, T entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null, string tableName = null) where T : class
        {
            ...
            var allKeyProperties = KeyPropertiesCache(type).Concat(ExplicitKeyPropertiesCache(type)).ToList().AsReadOnly();

            if (allKeyProperties.Count == 0)
            ...
            var allProperties = TypePropertiesCache(type);
            var computedProperties = ComputedPropertiesCache(type);
            var nonIdProps = allProperties.Except(allKeyProperties.Union(computedProperties)).ToList();
            ...
            for (var i = 0; i < allKeyProperties.Count; i++)
            {
                ...
                //     keyName = keyName.ToUnderscore();
                // }
                adapter.AppendColumnNameEqualsValue(sb, keyName);  //fix for issue #336
        ...
        public static bool Delete<T>(this IDbConnection connection, T entityToDelete, IDbTransaction transaction = null, int? commandTimeout = null, string tableName = null) where T : class
        {
        ...
        public static bool DeleteAll<T>(this IDbConnection connection, IDbTransaction transaction = null, int? commandTimeout = null, string tableName = null) where T : class
        {
            var type = typeof(T);
            var name = tableName ?? GetTableName(type);
        ...
        internal static class ProxyGenerator
        {
            ...
            public static T GetInterfaceProxy<T>()
            {
                ...
                var typeBuilder = moduleBuilder.DefineType(typeOfT.Name + "_" + Guid.NewGuid().ToString(),
            ...
            private static MethodInfo CreateIsDirtyProperty(TypeBuilder typeBuilder)
            {
                ...
                var currGetPropMthdBldr = typeBuilder.DefineMethod("get_IsDirty",
                                             getSetAttr,
                                             propType,
                                             Type.EmptyTypes);
                ...
                var currSetPropMthdBldr = typeBuilder.DefineMethod("set_IsDirty",
                                             getSetAttr,
                                             null,
                                             new[] { propType });
                ...
                var getMethod = typeof(IProxy).GetMethod("get_IsDirty");
                var setMethod = typeof(IProxy).GetMethod("set_IsDirty");
    ...
    public partial class SqlServerAdapter : ISqlAdapter
    {
        ...
        public void AppendColumnName(StringBuilder sb, string columnName)
        {
            if (DefaultTypeMap.MatchNamesWithUnderscores)
            {
                columnName = columnName.ToUnderscore();
            }
        ...
        {
            var paraName = columnName;
            if (DefaultTypeMap.MatchNamesWithUnderscores)
            {
                columnName = columnName.ToUnderscore();
            }
            sb.AppendFormat("[{0}] = @{1}", columnName, paraName);
        }
    }
        ...
        public int Insert(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, string tableName, string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert)
        {
            ...
            else
            {
                ...
                foreach (var property in propertyInfos)
                {
                    ...
                    if (DefaultTypeMap.MatchNamesWithUnderscores)
                    {
                        sb.Append(property.Name.ToUnderscore());
                    }
                    else
                    {
            ...
            foreach (var p in propertyInfos)
            {
                ...
                if (keyProperties.Contains(p) && id == 0)
