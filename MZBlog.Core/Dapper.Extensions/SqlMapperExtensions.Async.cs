...
namespace Dapper.Extensions
{
    public static partial class SqlMapperExtensions
    {
        ...
        {
            var result = await connection.QueryAsync(sql, null, transaction, commandTimeout).ConfigureAwait(false);
            var list = new List<T>();
        ...
        public static async Task<bool> UpdateAsync<T>(this IDbConnection connection, T entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null, string tableName = null) where T : class
        {
            ...
            for (var i = 0; i < allKeyProperties.Count; i++)
            {
                ...
                //     keyName = keyName.ToUnderscore();
                // }
                adapter.AppendColumnNameEqualsValue(sb, keyName);
        ...
        {
            var type = typeof(T);
            var statement = "DELETE FROM " + (tableName ?? GetTableName(type));
    ...
    public partial class FbAdapter
    {
        ...
        {
            var cmd = $"insert into {tableName} ({columnList}) values ({parameterList})";
            var rows = await connection.ExecuteAsync(cmd, entityToInsert, transaction, commandTimeout).ConfigureAwait(false);
